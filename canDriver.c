...
void CAN_RX_init(void){
    ...
    CAN0_IF2ARB2_R &= ~(CAN_IF2ARB2_XTD)| CAN_IF2ARB2_DIR; // 11 bit ID, receive mode
    CAN0_IF2ARB2_R |= 1<<2 ; // set message ID to 1 for 11 bit ID to be received
    CAN0_IF2ARB2_R |= CAN_IF2ARB2_MSGVAL; // the message is valid and ready to be considered
    
    CAN0_IF2MCTL_R &= ~CAN_IF2MCTL_RMTEN; // disable remote frame response
    CAN0_IF2MCTL_R |= CAN_IF2MCTL_EOB|1; // set for Single message object with EOB and 1 byte data frame
    
    CAN0_IF2CMSK_R |= 0x007F; // stop writing to the interface registers, pull all data from the messages received
